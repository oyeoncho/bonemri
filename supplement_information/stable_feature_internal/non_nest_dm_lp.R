...
base_dir <- "image_data/survival_model/mixture_non_fix/non_nest/beit0/results/generalization/test3/dl0"
...
# ----------------------------------------
# (A) 그림 1: C-index by Feature set (n7, dm_lp)
# ----------------------------------------
p_cindex_n7_dm_lp <- ggplot(all_results_n7,
                         aes(x = Feature, y = mean_cindex_val, fill = Feature)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3) +
  scale_fill_brewer(palette = "Set2", name = "Feature Set") +
  labs(
    title    = "Validation C-index by Feature Set (DM+LP)",
...
print(p_cindex_n7_dm_lp)

# ----------------------------------------
# (B) 그림 2: AUC by Feature set (n7, dm_lp+LP)
# ----------------------------------------
p_auc_n7_dm_lp <- ggplot(all_results_n7,
                      aes(x = Feature, y = mean_AUC_val, fill = Feature)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3) +
  scale_fill_brewer(palette = "Set2", name = "Feature Set") +
  labs(
    title    = "Validation AUC by Feature Set (DM+LP)",
...
print(p_auc_n7_dm_lp)

# 필요하면 저장
# ggsave("dm_lp_n7_cindex_by_feature.png", p_cindex_n7_dm_lp, width = 8, height = 6, dpi = 300)
# ggsave("dm_lp_n7_auc_by_feature.png",    p_auc_n7_dm_lp,    width = 8, height = 6, dpi = 300)
...
# 1) 요약 (mean / SD)
summary_n7_dm_lp <- all_results_n7 %>%
  group_by(Feature) %>%
  summarise(
...
boot_ci_n7_dm_lp <- all_results_n7 %>%
  group_by(Feature) %>%
  summarise(
    # C-index bootstrap
...
# 4) 최종 요약 테이블 (논문용 포맷)
summary_with_ci_n7_dm_lp <- summary_n7_dm_lp %>%
  left_join(boot_ci_n7_dm_lp, by = "Feature") %>%
  mutate(
...
print(summary_with_ci_n7_dm_lp)

# 필요하면 CSV 저장
 write.csv(summary_with_ci_n7_dm_lp,
           "dm_lp_n7_feature_summary_with_bootstrap_CI.csv",
           row.names = FALSE)
...
pw_cindex_n7_dm_lp <- pairwise.wilcox.test(
...
# AUC에 대한 쌍별 Wilcoxon
pw_auc_n7_dm_lp <- pairwise.wilcox.test(
...
p_cindex_clin_img    <- pw_cindex_n7_dm_lp$p.value["Image only",       "Clinical only"]
p_cindex_img_imgClin <- pw_cindex_n7_dm_lp$p.value["Image + Clinical", "Image only"]

p_auc_clin_img       <- pw_auc_n7_dm_lp$p.value["Image only",          "Clinical only"]
p_auc_img_imgClin    <- pw_auc_n7_dm_lp$p.value["Image + Clinical",    "Image only"]

p_table_n7_dm_lp <- tibble::tibble(
...
print(p_table_n7_dm_lp)

# 필요하면 CSV로 저장
 write.csv(p_table_n7_dm_lp,
           "dm_lp_n7_pairwise_pvalues.csv",
           row.names = FALSE)
