...
base_dir <- "image_data/survival_model/mixture_non_fix/non_nest/beit0/results/generalization/test4/dl0"
...
# ----------------------------------------
# (A) 그림 1: C-index by Feature set (n7, LP)
# ----------------------------------------
p_cindex_n7_lp <- ggplot(all_results_n7,
                         aes(x = Feature, y = mean_cindex_val, fill = Feature)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3) +
  scale_fill_brewer(palette = "Set2", name = "Feature Set") +
  labs(
    title    = "Validation C-index by Feature Set (LP)",
    subtitle = "n7 (2 image features) with 494-patient cohort",
...
print(p_cindex_n7_lp)

# ----------------------------------------
# (B) 그림 2: AUC by Feature set (n7, LP)
# ----------------------------------------
p_auc_n7_lp <- ggplot(all_results_n7,
                      aes(x = Feature, y = mean_AUC_val, fill = Feature)) +
  geom_boxplot(alpha = 0.8, outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.3) +
  scale_fill_brewer(palette = "Set2", name = "Feature Set") +
  labs(
    title    = "Validation AUC by Feature Set (LP)",
    subtitle = "n7 (2 image features)with 494-patient cohort",
...
# 필요하면 저장
# ggsave("lp_n7_cindex_by_feature.png", p_cindex_n7_lp, width = 8, height = 6, dpi = 300)
# ggsave("lp_n7_auc_by_feature.png",    p_auc_n7_lp,    width = 8, height = 6, dpi = 300)
...
# 4) 최종 요약 테이블 (논문용 포맷)
summary_with_ci_n7_lp <- summary_n7_lp %>%
  left_join(boot_ci_n7_lp, by = "Feature") %>%
  mutate(
...
print(summary_with_ci_n7_lp)

# 필요하면 CSV 저장
write.csv(summary_with_ci_n7_lp,
           "lp_n7_feature_summary_with_bootstrap_CI.csv",
           row.names = FALSE)
...
pw_cindex_n7_lp <- pairwise.wilcox.test(
...
# AUC에 대한 쌍별 Wilcoxon
pw_auc_n7_lp <- pairwise.wilcox.test(
...
p_cindex_clin_img    <- pw_cindex_n7_lp$p.value["Image only",       "Clinical only"]
p_cindex_img_imgClin <- pw_cindex_n7_lp$p.value["Image + Clinical", "Image only"]

p_auc_clin_img       <- pw_auc_n7_lp$p.value["Image only",          "Clinical only"]
p_auc_img_imgClin    <- pw_auc_n7_lp$p.value["Image + Clinical",    "Image only"]

p_table_n7_lp <- tibble::tibble(
...
# 필요하면 CSV로 저장
 write.csv(p_table_n7_lp,
           "lp_n7_pairwise_pvalues.csv",
           row.names = FALSE)
