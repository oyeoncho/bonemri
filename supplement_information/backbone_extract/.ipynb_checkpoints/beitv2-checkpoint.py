...
CSV_PATH = "./cx.csv"
MODEL_PATH = "./pretrained_models/beitv2/best_model_overall.pt"
SAVE_DIR = "./models/beitv2"
...
def extract_masked_slices(vol, mask):
    mask = (mask >= 0.5).astype(np.float32)
    slices = [vol[:, :, i] * mask[:, :, i] for i in range(vol.shape[2]) if np.sum(vol[:, :, i] * mask[:, :, i]) > 0]
    if not slices:
        return [np.zeros_like(vol[:, :, 0])] * MAX_SLICES
    if len(slices) >= MAX_SLICES:
        ...
        pad_count = MAX_SLICES - len(slices)
        slices += [np.zeros_like(vol[:, :, 0])] * pad_count
    return slices
...
class SurvivalSliceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(cache_path):
            ...
            vol = nib.load(os.path.join(IMAGE_DIR, f"{pid}.nii.gz")).get_fdata().astype(np.float32)
            mask = nib.load(os.path.join(MASK_DIR, f"{pid}.nii.gz")).get_fdata().astype(np.float32)
            slices = extract_masked_slices(vol, mask)
            imgs = torch.stack([slice_to_tensor(s) for s in slices])
...
for run in range(NUM_RUNS):
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if epoch_loss < best_loss - 1e-4:
            ...
            }, os.path.join(SAVE_DIR, f"best_model_beitv2_run{run}.pt"))
...
if best_overall_model_state is not None:
    torch.save(best_overall_model_state, os.path.join(SAVE_DIR, "best_model_beitv2_overall.pt"))
