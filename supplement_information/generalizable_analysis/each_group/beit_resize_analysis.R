...
for (grp in groups) {
  group_results <- map_dfr(1:30, function(i) {
    auc_path <- sprintf("image_data/survival_model/mixture_non_fix/tune/beit_resize/results/generalization/test/dl0/%s/raw_auc_per_time_run%02d.csv", grp, i)
    cindex_path <- sprintf("image_data/survival_model/mixture_non_fix/tune/beit_resize/results/generalization/test/dl0/%s/raw_cindex_per_time_run%02d.csv", grp, i)
...
ggplot(all_results, aes(x = Feature, y = mean_cindex_val, fill = Feature)) +
  ...
  labs(
    title = "Validation C-index by Feature Set and Group (Beit_resize)",
...
ggplot(filtered_plot_data, aes(x = group, y = Value, fill = Metric)) + 
  ...
  labs(
    title = paste("Run-wise Metrics for", target_feature, "(Beit_resize)"),
    subtitle = "Validation C-index of selected features",
    y = "Value", 
