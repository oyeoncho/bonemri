...
for (grp in groups) {
  group_results <- map_dfr(1:30, function(i) {
    auc_path <- sprintf("image_data/survival_model/mixture_non_fix/tune/beit0/results/generalization/test/dl0/%s/raw_auc_per_time_run%02d.csv", grp, i)
    cindex_path <- sprintf("image_data/survival_model/mixture_non_fix/tune/beit0/results/generalization/test/dl0/%s/raw_cindex_per_time_run%02d.csv", grp, i)
