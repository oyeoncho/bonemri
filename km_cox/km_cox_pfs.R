...
OUT_DIR <- "./image_data/survival_model/mixture_non_fix/non_nest/beit0/results/reviewer_km_pack_pfs/dl0/n7_30_30/file04_run06"
...
uv_path <- file.path(OUT_DIR, "Cox_INTERNAL_ALL_T60_stageBINCATEG_imgrisk_per0p1_UV.csv")
mv_path <- file.path(OUT_DIR, "Cox_INTERNAL_ALL_T60_stageBINCATEG_imgrisk_per0p1_MV.csv")
...
pretty_term <- function(term) {
  term %>%
    ...
    str_replace("^stage_bin_1$", "Stage (IIIC2–IVB)") %>%
    str_replace("^imaging_risk$", "Imaging risk (per 0.1 increase)") %>%
...
p_both <- ggplot(cox_all, aes(x = HR, y = term_pretty)) +
  ...
  geom_point(aes(color = model),
             ...
             position = position_dodge(width = 0.55)) +
  scale_x_log10() +
  labs(title = "Cox model (Progression-free survival)",
